...
# Connection is thread-local (one per executor worker) to avoid reconnecting
# on every query.
...
async def save_session(sess: EditorSession) -> None:
    # Idempotent upsert; regenerate share_code on the rare collision.
    for _ in range(3):
        try:
            await db_exec(
...
async def find_active_session_for_user(user_id: int, channel_id: int) -> EditorSession | None:
    # Most recently active session for this user in this channel.
    try:
        rows = await db_exec(
