...
async def check_resources() -> tuple[bool, str]:
    # cpu_percent(interval=None) is instant (delta since last call), unlike
    # interval=0.1 which would block the event loop.
    try:
...
def _build_cmd(
    ...
    # tl_dict is a snapshot so UI mutations mid-render can't break the filtergraph.
    # overlay_files are pre-written text files for drawtext=textfile=.
    inputs: list[str] = []
    filters: list[str] = []
    ...
    for idx, ov in enumerate(overlays):
        ...
        y = ov.get("y", "50")
        # x/y are ffmpeg expressions but come from Overlay defaults, not user input.
        # If a coordinate input field is ever added, sanitize it here.
...
async def _drain_stderr(proc: asyncio.subprocess.Process) -> bytes:
    # Drain the stderr pipe so ffmpeg doesn't block on write() with large errors.
...
async def background_render(sess: EditorSession, interaction: discord.Interaction) -> None:
    ...
    # Snapshot the timeline so undo/redo during render can't break build_cmd.
    ...
    # Write overlay text files once per render; cleaned up in finally.
    overlay_files: list[str] = []
    ...
    try:
        async with sem:
            while attempts <= 5:
                if sess.cancel_flag.is_set():
                    break
                # Regenerate overlay files if something wiped them (idempotent).
                for idx, ov in enumerate(tl_dict.get("overlays", [])):
                ...
                try:
                    # Parse progress from stdout and drain stderr in parallel.
