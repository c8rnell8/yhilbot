...
def predict_size_mb(tl: Timeline) -> float:
    ...
    # 0.16 is an empirical libwebp factor at quality~75, 720p/15fps baseline.
    return max(0.3, total_dur * tl.fps * ((tl.width / 720) ** 2) * (tl.quality / 75) * 0.16)


class EditorView(ui.View):
    """Owner-facing editor view with buttons.

    Read-only access (/edit join_code=...) is shown without a view, as a separate
    embed, so a non-owner can look but not touch.
    """
    ...
    async def _edit_session_message(self, **kwargs) -> None:
        # Edit the editor message via the stored reference. For modal submits,
        # interaction.edit_original_response() targets the modal's deferred
        # response, not the editor message.
    ...
    async def _safe_edit(self, **kwargs) -> None:
        # Rate-limit message edits to about once per 1.5s.
    ...
    async def update_ui(self, note: str = "") -> None:
        # Only call after a locked mutation of the timeline.
    ...
    async def update_ui_from_modal(
        ...
        # Modal submit is already deferred; update_ui edits self.sess.message
        # directly instead of touching interaction-bound APIs.
...
def build_readonly_embed(sess: EditorSession) -> discord.Embed:
    # Snapshot embed for read-only viewing via join_code - no buttons.
    tl = sess.timeline
    clip = tl.clips[0] if tl.clips else None
    pred = predict_size_mb(tl)
    embed = discord.Embed(
