...
async def gen_preview_frame(sess: EditorSession) -> bytes | None:
    # Single JPEG at the cursor position, cached by (source, round(cursor, 1))
    # so dragging the cursor doesn't spawn an ffmpeg call per click.
...
async def extract_dominant_color(frame: bytes) -> discord.Color:
    # Average color of the frame, used for the embed accent.
    try:
