"""Глобальная очередь FIFO для /gif (и любой другой `await queue_wait`).

`queue_wait` возвращает True, когда задача дошла до головы очереди и удалена из неё,
False — если interaction истёк/таймаут (10 минут).
"""
...
async def queue_wait(interaction: discord.Interaction) -> bool:
    """Ставит interaction в очередь и ждёт своей позиции."""
...
async def queue_remove(interaction_id: int) -> None:
    """Идемпотентное удаление из очереди (fallback при ошибках)."""
    async with queue_lock:
        try:
