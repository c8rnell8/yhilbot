"""PIL-рендер мем-капшена для /caption."""
...
if not IMPACT_FONT_PATH:
    log.warning("⚠️ Impact не найден — /caption использует fallback-шрифт")
...
def draw_caption_sync(img_bytes: bytes, text: str, img_format: str) -> bytes:
    """Накладывает классический белый-фон-чёрный-текст caption поверх картинки.

    Вызывается из executor — не из event loop'а напрямую.
    """
